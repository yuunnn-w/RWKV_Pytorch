...
class RWKV_Block(nn.Module):
    ...
    def time_mixing_parallel(
        ...
        xxx = torch.tanh(xxx @ self.att_time_maa_w1).view(
            batch_size, L, 5, 1, -1
        )  # att_time_maa_w1: [2048, 160]
        ...
        # 使用注意力机制更新状态
        state_s = torch.empty(
            batch_size, L, H, S, S, device=x.device
        )  # 初始化state_s的结果张量
        s = state[:, (2 + S) * i + 2 : (2 + S) * (i + 1)].view(batch_size, H, S, S)
        ...
        if self.onnx_opset >= 18:
            x = (
                self.att_group_norm(
                    x.flatten(start_dim=2).view(batch_size * L, -1)
                ).view(batch_size, L, -1)
                * g
            )
        else:
    ...
    def forward_parallel(
        ...
        if self.onnx_opset >= 17:
            ...
            x = x + self.time_mixing_parallel(
                self.manual_layer_norm(x, self.ln1_weight, self.ln1_bias, 1e-5),
                state,
                i,
            )
            x = x + self.channel_mixing_parallel(
                self.manual_layer_norm(x, self.ln2_weight, self.ln2_bias, 1e-5),
                state,
                i,
            )
